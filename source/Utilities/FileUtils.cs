...
namespace Kyameru.Component.File.Utilities
{
    ...
    {
        public void WriteAllBytes(string path, byte[] file) => System.IO.File.WriteAllBytes(path, file);

        public void Move(string source, string destination) => System.IO.File.Move(source, destination);

        public void CreateDirectory(string directory) => System.IO.Directory.CreateDirectory(directory);

        public void CopyFile(string source, string destination) => System.IO.File.Copy(source, destination);
